<?php
    ...
    class Model {
        ...
        function createDataBase() {
            $database = new PDO('mysql:host=' . MYSQL_HOST, MYSQL_USER, MYSQL_PASS);
            $database->exec('CREATE DATABASE IF NOT EXISTS ' . MYSQL_DB);
        }
        ...
        function deploy() {
            ...
            if(count($tables) == 0) {
                ...
                $sql = <<<END
                    ...
                    ALTER TABLE `productos` ADD `oferta` BOOLEAN NOT NULL DEFAULT FALSE;

                    UPDATE `productos` SET `oferta` = TRUE WHERE `producto_id` IN (13, 5, 2, 14, 10);
